import { describe, expect, test } from "bun:test"
import { createIndexedStore } from "@signals"
import { createEffect, createRoot } from "solid-js"

const tick = (): Promise<void> => new Promise((resolve) => setTimeout(resolve, 0))

interface Quote {
  symbol: string
  price: number
}

const aapl: Quote = { symbol: "AAPL", price: 180 }
const msft: Quote = { symbol: "MSFT", price: 410 }

describe("createIndexedStore", () => {
  test("starts empty by default", () => {
    createRoot(() => {
      const s = createIndexedStore<string, Quote>()
      expect(s.size()).toBe(0)
      expect(s.keys()).toEqual([])
      expect(s.get("AAPL")).toBeUndefined()
    })
  })

  test("seeds from initial map", () => {
    createRoot(() => {
      const s = createIndexedStore<string, Quote>({ AAPL: aapl })
      expect(s.size()).toBe(1)
      expect(s.get("AAPL")?.price).toBe(180)
    })
  })

  test("set inserts a new key", () => {
    createRoot(() => {
      const s = createIndexedStore<string, Quote>()
      s.set("AAPL", aapl)
      expect(s.get("AAPL")).toEqual(aapl)
      expect(s.size()).toBe(1)
    })
  })

  test("set overwrites an existing key wholesale", () => {
    createRoot(() => {
      const s = createIndexedStore<string, Quote>({ AAPL: aapl })
      s.set("AAPL", { symbol: "AAPL", price: 200 })
      expect(s.get("AAPL")?.price).toBe(200)
    })
  })

  test("update shallow-merges into an existing entry", () => {
    createRoot(() => {
      const s = createIndexedStore<string, Quote>({ AAPL: aapl })
      s.update("AAPL", { price: 199 })
      expect(s.get("AAPL")?.symbol).toBe("AAPL")
      expect(s.get("AAPL")?.price).toBe(199)
    })
  })

  test("update is a no-op for missing keys", () => {
    createRoot(() => {
      const s = createIndexedStore<string, Quote>()
      s.update("AAPL", { price: 199 })
      expect(s.get("AAPL")).toBeUndefined()
      expect(s.size()).toBe(0)
    })
  })

  test("upsert inserts when missing", () => {
    createRoot(() => {
      const s = createIndexedStore<string, Quote>()
      s.upsert("AAPL", aapl)
      expect(s.get("AAPL")).toEqual(aapl)
      expect(s.size()).toBe(1)
    })
  })

  test("upsert shallow-merges when present", () => {
    createRoot(() => {
      const s = createIndexedStore<string, Quote>({ AAPL: aapl })
      s.upsert("AAPL", { symbol: "AAPL", price: 211 })
      expect(s.get("AAPL")?.price).toBe(211)
    })
  })

  test("remove deletes an entry", () => {
    createRoot(() => {
      const s = createIndexedStore<string, Quote>({ AAPL: aapl })
      s.remove("AAPL")
      expect(s.get("AAPL")).toBeUndefined()
      expect(s.size()).toBe(0)
    })
  })

  test("remove on missing key is a no-op", () => {
    createRoot(() => {
      const s = createIndexedStore<string, Quote>()
      s.remove("AAPL")
      expect(s.size()).toBe(0)
    })
  })

  test("replace swaps the entire map", () => {
    createRoot(() => {
      const s = createIndexedStore<string, Quote>({ AAPL: aapl })
      s.replace({ MSFT: msft })
      expect(s.get("AAPL")).toBeUndefined()
      expect(s.get("MSFT")).toEqual(msft)
      expect(s.size()).toBe(1)
    })
  })

  test("clear empties the store", () => {
    createRoot(() => {
      const s = createIndexedStore<string, Quote>({ AAPL: aapl, MSFT: msft })
      expect(s.size()).toBe(2)
      s.clear()
      expect(s.size()).toBe(0)
      expect(s.keys()).toEqual([])
    })
  })

  test("entries returns the live record", () => {
    createRoot(() => {
      const s = createIndexedStore<string, Quote>({ AAPL: aapl })
      const all = s.entries()
      expect(all.AAPL).toEqual(aapl)
    })
  })

  test("reactivity: createEffect re-runs on set/update/remove", async () => {
    await new Promise<void>((resolve) => {
      createRoot(async (dispose) => {
        const s = createIndexedStore<string, Quote>()
        const sizes: number[] = []
        createEffect(() => {
          sizes.push(s.size())
        })
        await tick()
        s.set("AAPL", aapl)
        await tick()
        s.set("MSFT", msft)
        await tick()
        s.update("AAPL", { price: 199 })
        await tick()
        s.remove("AAPL")
        await tick()
        expect(s.size()).toBe(1)
        // Effect ran for at least: initial + each mutation that changed dependencies.
        expect(sizes.length).toBeGreaterThan(1)
        dispose()
        resolve()
      })
    })
  })
})
